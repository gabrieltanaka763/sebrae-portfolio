...
function updateFilterOptions() {
    ...
    const dataToFilter = tableData.filter(row => {
        return (
            ...
            (selectedFilters.portfolio.length === 0 || selectedFilters.portfolio.includes(row['PORTFÓLIO']))
        );
    });

    const areas = [...new Set(tableData.map(item => item['ÁREA']))];
    const subAreas = [...new Set(dataToFilter.map(item => item['SUBÁREA']))];
    const portfolios = [...new Set(dataToFilter.map(item => item['PORTFÓLIO']))];
...
function applyFilters() {
    filteredData = tableData.filter(row => {
        ...
        const matchesPortfolio = selectedFilters.portfolio.length === 0 || selectedFilters.portfolio.includes(row['PORTFÓLIO']);
...
document.getElementById('downloadBtn').addEventListener('click', async () => {
    await exportToExcelWithCharts(filteredData, 'tabela_produto.xlsx');
});
